...
2. Run `generate_flops.py` or `generate_distinct_flops.py` in `src/python/` to produce `.gto` files in `generated/`.
3. Validate outputs using `gto_file_validate.py` before committing or publishing.

...
3. Run `generate_study_folders.py` to create a standardized target folder tree before solver exports.
4. Use `tools/ahk/test_file_create.ahk` to automate GTO+ with your template and generate one `.gto` file per flop into `generated/` (or your configured output directory).
5. Validate outputs using `gto_file_validate.py` before committing or publishing.
